...
export class GameScene extends Phaser.Scene {
    ...
    createPlayerBlueprint() {
        // Create a player blueprint from GameConfig
        // This is a temporary solution until a proper player blueprint is added to data/blueprints
        ...
        return {
            ...
            display: {
                texture: 'player',
                frame: 0,
                tint: GameConfig.player.color
            },
            stats: {
                hp: GameConfig.player.baseHP,
                speed: GameConfig.player.baseSpeed * 120, // Convert to pixels per second
                size: GameConfig.player.size
            },
            mechanics: {
                attack: {
                    intervalMs: GameConfig.player.projectileInterval
                },
                projectile: {
                    ref: 'player_projectile',
                    count: GameConfig.player.baseProjectiles,
                    spreadDeg: 15,
                    stats: {
                        damage: GameConfig.player.projectileDamage,
                        speed: GameConfig.player.projectileSpeed || 150,
                        range: GameConfig.player.projectileRange || 600
                    }
                },
                crit: {
                    chance: 0.05,
    ...
    generatePlayerTexture() {
        ...
        graphics.fillStyle(GameConfig.player.color, 1);
    ...
    generateEnemyTexture(textureKey, color, size = 20) {
        ...
        if (isBoss) {
            ...
            graphics.fillStar(size/2, size/2, 5, size/6, size/8);
        } else if (isUnique) {
    ...
    update(time, delta) {
        ...
        // No manual update needed, Phaser calls preUpdate() automatically
