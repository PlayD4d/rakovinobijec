...
import LootDropManagerTests from '../tests/LootDropManagerTests.js';
...
export default class FullSystemValidator {
  ...
  async runFullValidation() {
    ...
    try {
      ...
      // 3. LootDropManager tests
      await this.runLootDropManagerTests();
  ...
  async runLootDropManagerTests() {
    console.log('\n--- LootDropManager Tests ---');
    
    const lootTests = new LootDropManagerTests();
    const testResults = await lootTests.runAllTests();
    
    this.results.integrationTests.lootDropManager = testResults;
    
    if (!testResults.success) {
      this.errors.push('LootDropManager tests failed');
    }
  }
